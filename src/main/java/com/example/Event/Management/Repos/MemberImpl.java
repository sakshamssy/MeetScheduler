...
public class MemberImpl implements MemberService1{
    ...
    public String addMember(MemberDTO memberDTO){
        ...
        Member member = new Member( memberDTO.getBITSid(), memberDTO.getName(), this.passwordEncoder.encode(memberDTO.getPassword()));
        Calendar calendar = new Calendar(member.getBITSid()); //generated calenadars for all three members using
    ...
    public LoginMessage loginMember(LoginDTO loginDTO){
        ...
        Member member1 = memberRepository.findMemberByBITSid(loginDTO.getBITSid());
        if(member1 != null){
            String password = loginDTO.getPassword();
            String encodedPassword = member1.getPassword();
            Boolean isPwdRight = passwordEncoder.matches(password, encodedPassword);
            if(isPwdRight) {
                Optional<Member> member = memberRepository.findOneByBITSidAndPassword(loginDTO.getBITSid(), encodedPassword);
                if (member.isPresent()) {
